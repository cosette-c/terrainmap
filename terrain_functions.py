...
def distances_from_origin(f,p,position,roll_pitch_yaw):
    ...
    for row in points:
        ...
        P1_matrix = np.vstack((P1_matrix,vector))

    P0_matrix = np.empty((1,4))

    for row in P1_matrix:
        P0_vector = np.dot(H, P1_matrix[row].transpose())
